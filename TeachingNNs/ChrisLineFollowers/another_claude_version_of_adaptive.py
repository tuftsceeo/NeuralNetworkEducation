...
Unlike the "make sure it works" version, this one skips the offline
pretraining burst on purpose: the network starts on truly random
weights, and the ONLY training it ever gets is the handful of backprop
steps it takes per tick, live, while it's already driving. The point
is to watch behavior evolve in real time -- expect it to spin, stall,
or wander for the first several seconds before it visibly starts
correcting toward the line as the weights converge.
...
opposite directions. RIGHT_SIGN below flips the right wheel's command
to compensate -- this is a hardware fact, not a network fact, so it's
applied at the motor call, not baked into the training targets.
"""
...
while True:
    ...
    motor.run_right(clamp(RIGHT_SIGN * 100 * y[1]))
    if TICK_SLEEP:
