...
def record(label_idx):
    ...
    for _ in range(NUM_SAMPLES):
        ...
        gx, gy, gz = imu.angular_velocity()# deg/s
        samples.extend([ax, ay, az, gx, gy, gz])
        time.sleep_ms(SAMPLE_RATE_MS)

    # Format: SAMPLE,<label_idx>,v0,v1,v2,...
    values = ",".join(f"{v:.4f}" for v in samples)
    print(f"SAMPLE,{label_idx},{values}")
...
while True:
    ...
    hub.light_matrix.write(gesture_name[0])# R, P, or S
    print(f"GESTURE,{gesture_idx},{gesture_name}")
