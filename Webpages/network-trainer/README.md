...
- **Devices card** — connect/disconnect LEGO hardware over BLE; connected devices appear as removable chips. On browsers without Web Bluetooth (Safari, Firefox) this is replaced by a virtual controller instead: two spring-loaded sliders (snap back to 0 on release, plus a "0 inputs" reset button) feed `LeftPercent`/`RightPercent` like a real Controller, and a spinning icon visualizes a `Speed` output like a real Motor — registered automatically, no connect step. A dismissible banner explains the swap.
...
- **Activation functions per layer** — None, ReLU, Sigmoid, Tanh, Softplus, or Custom (piecewise expression editor), with a help popup. Hidden by default so first-touch students see a plain weighted-sum network; a "+ Activation function" link reveals it for every layer at once.
...
| `main.py` | Wires all event handlers, defines the Play/Stop loop, boots the initial 1-input/1-layer/1-output network, and checks for Web Bluetooth support at boot. |
| `Device.py` | LEGO hardware abstraction via the `legoeducation` package plus BLE glue and WASM/single-thread compatibility patches. Also defines `VirtualDevice`, the slider/motor stand-in used when Web Bluetooth is unavailable. |
...
| `misc_ui/ui_chrome.py` | Zoom controls, the activation-column show/hide toggle, and resize-observer-based layout upkeep. |
...
Hosted at [pyscript.com/@tuftsceeo/network-trainer](https://pyscript.com/@tuftsceeo/network-trainer). See the [repo-level PyScript deployment notes](../../README.md#how-the-webpages-apps-deploy-to-pyscriptcom) for the full picture — short version: every `.py`/`.js` file here is fetched live from `raw.githubusercontent.com`/jsdelivr on `main`, so pushing to `main` is enough to update those. `pyscript.toml`, `index.html`, and `styles.css`, however, live only in the pyscript.com project's own storage and have to be hand-copied there after any change — the copies in this directory are reference mirrors, not the live ones. Forgetting this trio is the single most common cause of "I pushed it but the live site didn't change."
